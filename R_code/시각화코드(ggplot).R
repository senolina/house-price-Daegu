...
gg1<-ggplot(data = house, aes(x=as.factor(DateBuilt), y=Price.per.Size ,group=DateBuilt ,fill=DateBuilt,alpha = 0.7)) + 
...
gg24<-ggplot(data = house, aes(x= HeatingType, fill=line ,alpha = 0.7)) + 
   geom_bar()  + 
   theme_bw() +  ##바탕 하얗게 하는 거
   scale_x_discrete(name = "난방 유형") 
gg24
...
gg25<-ggplot(data = house, aes(x=AptManageType, fill=line ,alpha = 0.7)) + 
   geom_bar()  + 
   theme_bw() +  ##바탕 하얗게 하는 거
   scale_x_discrete(name = "아파트 관리 유형") 
gg25
