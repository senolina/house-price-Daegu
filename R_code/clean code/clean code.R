...
setwd("C:/Users/yongheon/Desktop/비어플/대구 집값/koreahousedata")
...
house$complex <- as.numeric(cut((house$N_Parkinglot.Basement. + house$N_Parkinglot.Ground.),
                                breaks = c(0, 500, 1000, 1500), include.lowest = T))
...
100 * prop.table(table(house$complex, house$SubwayStation), margin = 1)  # 조건부 비율(complex 별 비율)
100 * prop.table(table(house$complex, house$SubwayStation))  # 전체 비율
