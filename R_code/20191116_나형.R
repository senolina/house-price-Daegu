...
qplot(N_SchoolNearBy.High., N_SchoolNearBy.University.,colour=cluster1$cluster,data = house[,18:27])
...
inTrain <- createDataPartition(y = house[, 18:27], p = 0.7, list = F)
training <- house[, 18:27][inTrain,]
testing <- house[, 18:27][-inTrain,]
training


###모델작성 (원래 그 전에 표준화가 필효하지만 현재 데이터는 개 수로 모두 단위가 같음)

house.kmeans <- kmeans(training, centers=2)##안돼,,,
...
nc<-NbClust(house[,18:27], min.nc = 2, max.nc=8, method="kmeans")##해석좀,,,
?NbClust


barplot(table(nc$Best.n[1,]))  ##2로 돌리라고?


##궁금한거 그냥 house[,18:27] 돌리면 돌아가는데 70% train은 안돌아갈까
...
library(corrplot)
corrplot(house, method="number")

corr<-cor(house[,-c(7:9,12:13,17)])
