...
set.seed(1234)
rf.fit = randomForest(Price.per.Size ~ ., data = train_tree, mtry = floor(ncol(train_tree) / 3), ntree = 150, importance = T)
rf.fit
...
# GradientBoosting
fit.gbm <- gbm(Price.per.Size ~ as.numeric(DateSold) + as.numeric(DateBuilt) + . - DateSold - DateBuilt, 
               distribution = "multinomial", n.trees = 500, verbose = T,
               interaction.depth = 5, shrinkage = 0.001, data = train_tree)
fit.gbm
...
write.csv(house , file="new_house.csv")