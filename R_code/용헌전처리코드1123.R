...
boxplot(house$Size.p. ~ house$complex) ###오 대강 그렇게 보이네욤
...
# 큰 차이는 없지만 2차항 모델이 꼬리부분에서 좀 더 정규성을 보이는 것으로 보임  ##이게 무슨 말이지...?
...
lines(lowess(x = house$DateBuilt, y = house$Price.per.Size), col = "red", lwd = 2)  ###이거 왜 안그려지지ㅜㅜ
...
# 따라서 sqrt(target)변수로 residual plot 확인 결과 increasing trend 제거된 것으로 확인   ##루트변환..?
...
# 지하철까지 거리를 고정시킨 상태에서 N_manager(단지 규모라고 생각함) 별 시간 제거한 상태의 평당 가격 확인   ##규모?에 해당하는 변수를 따로 만들면 안되나..?(x 변수들 끼리의 corr 고려)=>N_변수들을 다 짬뽕해서 규모 변수?
...
boxplot(house$SalePrice ~ house$group)  ##얘만 11단지 이상한데..? 왜 갑자기 훅 작아지지? 되게 싼데 평당 가격은 비쌈..?
...
boxplot(aggregate(house$SalePrice_Time ~ house$group, FUN = var)[, 2] / 
          aggregate(house$Price.per.Size ~ house$group, FUN = var)[, 2] ~ c(1:26), ylim = c(0, 1))  ###박스플랏이 아닌데,,? 뭐지 이 직선들은
...
table(subset(house, TimeToSubway == "15min~20min", SubwayStation))
table(subset(house, SubwayStation == "no_subway_nearby", c(TimeToSubway)))

unique(subset(house, TimeToSubway != "no_subway_station_nearby" & SubwayStation == "no_subway_nearby", select = group))

subset(house,group==25)
...
###범주=>수치화 하는 이유? 자유도를 낮춰주기 위해 

house[,SubwayStation]
house$N_Parkinglot.<- house$N_Parkinglot.Ground. + house$N_Parkinglot.Basement.
str(house$N_Parkinglot.)

tmp <- scale(house$N_Parkinglot., center = F) / scale(house$N_APT, center = F) * scale(house$N_FacilitiesInApt, center = F)

table(house$N_FacilitiesInApt)
table(house$N_APT)
summary(tmp)
subset(house$N_Parkinglot.Ground.,house$group==25)


subset(house,group==11)
...
subset(house,TimeToSubway=="no_subway_station_nearby"&group==6)
nrow(subset(house,group==6))




#주택의 특성 - 건축일자, 평수, 층, 복도타입, 난방타입, 엘레베이터 수, 관리자 수, (지하주차장 수 , 지상주차장 수),아파트 동 수,n_facilty

#사회기반 시설 - 지하철, TTBus, TTSub,

#내적요인 :  아파트규모-주차장 수 , N_APT, N_man,(( N_el / 동 수?)) 고려해보자! , N_facinAPT/동 수 
